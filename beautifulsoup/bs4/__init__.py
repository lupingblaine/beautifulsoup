...
class BeautifulSoup(Tag):
    ...
    def __iter__(self) -> Iterator[PageElement]:
        """Iterate over all nodes in the parse tree rooted at this soup.

        This performs a depth-first, pre-order traversal over all
        descendants of this BeautifulSoup object, yielding one node at
        a time. The traversal is implemented as a generator and does
        not materialize the full list of nodes up front, which keeps
        memory usage low.
        """
        ...
        # 从 soup 的顶层子节点开始遍历，不把 soup 自己算作一个节点
